...
namespace knights {
    ...
    class RobotController {
        private:
            ...
            void run_follow_route(const knights::Route &route, float lookahead_distance = 15.0, const float max_speed = 127.0, bool forwards = true, float end_tolerance = 4.0, float timeout = 5000, bool use_pid = true);

            void run_follow_profile(const knights::MotionProfile &profile, float end_tolerance = 6.0, bool forwards = true);

            void run_lateral_to_position(const Pos desired_position, const bool forwards = true, const float end_tolerance = 3.5, const int timeout = 1000, const int turn_timeout = 750, const int wait_time = 140);

            void run_lateral_to_point(const Pos desired_position, const bool forwards = true, const float end_tolerance = 3.5, const int timeout = 1000, const int turn_timeout = 750, const int wait_time = 140);

            void run_curve_move(const knights::Pos point, const bool forwards = true, const float end_tolerance = 2.0, float timeout = 1250, bool async= false);

            void run_move_to_position(const Pos desired_position, float lead = 0.5, float correction_dist = 8.0, const float &end_tolerance = 2.0, const bool forwards = true, float timeout = 1000);
             ...
             */
            void run_turn_to_angle(const float angle, int direction = 0, float end_tolerance = 3.0, int timeout = 2000, bool rad = false); // DEGREES

            void run_right_swing_to_angle(const float angle, float end_tolerance = 3.0, int timeout = 2000, bool rad = false); // DEGREES

            void run_left_swing_to_angle(const float angle, float end_tolerance = 3.0, int timeout = 2000, bool rad = false); // DEGREES
             ...
             * @param timeout 
             */
            void run_turn_to_point(knights::Pos point, bool forwards = true, int direction = 0, float end_tolerance = 3.0, int timeout = 2000); // DEGREES
             ...
             */
            void run_lateral_move(const float distance, float end_tolerance = 3.0, float timeout = 1500, bool async = false);
             ...
             * @param rad Whether the provided angle is in radians or not
             */
            void run_turn_for(const float angle, const float end_tolerance = 2.0, float timeout= 750, bool rad = false); // DEGREES
    };
        ...
        public:
            ...
            RobotController(RobotChassis *chassis, PIDController *lateral_pid, PIDController *turn_pid, PIDController *angular_pidu bool use_motor_encoders = false);
